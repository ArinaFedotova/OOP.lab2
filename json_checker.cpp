...
void Json_checker::check_json(const std::string& json_text) {
    skip_empty_space(json_text); // пропускаем пробелы вначале
...
void Json_checker::check_special_type(const std::string& json_text) {
    ...
    if (json_text[string_index] == 't' || json_text[string_index] == 'T')
        expected_word = "true";
    else if (json_text[string_index] == 'f' || json_text[string_index] == 'F')
        expected_word = "false";
    else
        expected_word = "null";
...
void Json_checker::check_object(const std::string& json_text) {
    if (is_okay == true) {
        string_index++;
        skip_empty_space(json_text);
        Expected_part part; // что ожидает получить
        ...
        while (json_text[string_index] && is_okay == true) {
            ...
            else if (part.current == part.value) {
                if (json_text[string_index] == '"')
                    check_string(json_text);
                else if (correct_symbols.find(json_text[string_index]) != std::string::npos)
                    check_number(json_text);
            ...
            else if ((part.current == part.end_of_object || part.current == part.end_of_line) && json_text[string_index] == '}')
                break;
            else {
                is_okay = false;
